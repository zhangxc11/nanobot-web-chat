...
- [ ] **T21.2** 前端 API 层扩展
  - 新增 `fetchSessionUsage(sessionKey)` 接口
  - 新增 `fetchDailyUsage(days)` 接口
  - 新增对应 TypeScript 类型

- [ ] **T21.3** UsageIndicator 改为 Session 用量
  - 接收 activeSessionId，查询当前 session 用量
  - 切换 session 时刷新
  - 展开显示输入/输出/按模型明细
...
- [ ] **T21.5** UsagePage 实现 — 总计卡片 + 按模型 + 按 Session
  - 调用 GET /api/usage 获取全局数据
  - 总计卡片（总 tokens、输入、输出、调用次数）
  - 按模型分布表格
  - 按 Session 分布表格

- [ ] **T21.6** UsagePage 实现 — 每日趋势图
  - 调用 GET /api/usage/daily 获取趋势数据
  - 纯 CSS/SVG 柱状图（不引入图表库）
