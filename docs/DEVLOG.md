...
- [ ] **T49.1** 后端 — `analytics.py` + `webserver.py` 增加 `period` 参数
  - `get_global_usage(period=None)` — 支持 `1d|7d|30d|all`
  - `get_daily_usage(period=None)` — 同上
  - `webserver.py` 解析 `?period=` query param 并传递

- [ ] **T49.2** 前端 API — `fetchUsage()` / `fetchDailyUsage()` 增加 `period` 参数

- [ ] **T49.3** 前端 — 时间段选择器 UI + 联动
...
- [ ] **T49.7** 前端 — 每日趋势曲线图
  ...
  - X 轴日期、Y 轴刻度、hover tooltip

- [ ] **T49.8** CSS 样式 + 构建 + Git 提交
