...
### 8.4 Gateway 修改的安全规则 (v1.4 新增)

> **背景**：2026-02-25 发生过一次事故 — nanobot 在 Web UI 任务中修改了 `gateway.py` 并重启 gateway，导致正在进行的 SSE 连接断开，前端报 "TypeError: Failed to fetch"，任务中断。

**规则**：
1. **绝不在 Web UI 任务执行过程中重启 gateway 或 worker** — 这会断开当前 SSE 连接
2. **修改 gateway.py 后不需要立即重启** — 修改不会自动生效，等用户手动重启即可
3. **如果 gateway 新增了 API 路由**，告知用户需要手动重启 gateway：
   - `kill <gateway_pid> && python3 gateway.py --port 8081 --worker-url http://127.0.0.1:8082 &`
   - 或直接重新运行 `start.sh`
