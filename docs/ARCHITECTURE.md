...
> **v2.5 改进**：引入 `--daemonize` 标志 + `restart-gateway.sh` 脚本，exec 工具可安全调用脚本重启服务。同时 exec 工具拒绝含 `&` 后台操作符的命令。
...
6. **安全重启方式**：`restart-gateway.sh` 脚本（内部使用 `--daemonize` double-fork）

**总结**：
| 修改内容 | 需要重启？ | 谁来重启？ | 降级保护？ |
|----------|-----------|-----------|-----------|
| 前端 .tsx/.css + vite build | ❌ 不需要 | — | — |
| gateway.py | ✅ 需要 | **用户手动** 或 `restart-gateway.sh` | ✅ Worker 继续执行 |
| worker.py | ✅ 需要 | **用户手动** 或 `restart-gateway.sh` | ❌ 会中断任务 |
...
**restart-gateway.sh 脚本**：
```bash
# 用法
./restart-gateway.sh [all|gateway|worker|stop|status]

# 环境变量
GATEWAY_PORT=8081  WORKER_PORT=8082  # 可覆盖端口
```

脚本流程：
1. 通过 PID 文件 (`/tmp/nanobot-{gateway,worker}.pid`) 检测运行状态
2. 发送 SIGTERM 停止旧进程，等待退出
3. 使用 `--daemonize` 启动新进程
4. 健康检查（`/api/health`），最多等待 5 秒
