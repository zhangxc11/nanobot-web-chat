...
>
> ⚠️ **迁移说明 (2026-03-16)**：Phase 42 中定义的前端启发式规则 `resolveParent()` 的解析逻辑，已迁移到 nanobot 核心 §53 (`session/parents.py`)。前端改为消费 `/api/sessions/tree` API，不再维护本地启发式代码。详见 web-chat §五十四。
...
>
> ⚠️ **迁移说明 (2026-03-16)**：本节定义的启发式规则 B 扩展（`endsWith('_' + ts)` 备选搜索），其解析逻辑已迁移到 nanobot 核心 §53 (`session/parents.py`)。前端改为消费 API，详见 web-chat §五十四。
